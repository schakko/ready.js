...
var r = {
  ...
  // If a file is a vows file
  isVows : function(file) {
    if (typeof(config.vows) == "string") { config.vows = [config.vows]; }
    var filename = file.substring(file.lastIndexOf("/")+1);
    return config.vows.indexOf(filename) >= 0
  },
  // Show formated jslint errors
  showJslintErrors : function (jslint) {
    jslint.errors.reverse().forEach(function(e) {
      if (e) {
        logger.warn([e.line.toString(), ",", e.character.toString(), " : ",

...
var r = {
  ...
  jslint : function(fileOrCode, callback) {
    r.getCode(fileOrCode, function(code) {
      var succeeded = jslint(code);
      callback({ "jslint": {"success": succeeded, "data": jslint}});
    });
  },
  // Run vows
  vows : function(files, callback) {
	var callback;
	var args = [config.vowsExec, "--spec"].concat(files);
	var cmd = args.join(" ");

	cp.exec(cmd, function(error, stdout, stderr) {
		var error, stdout, stderr;
		callback({"vows": {"success": error, "data": stdout, "error": stderr}});
	});
  },
  // Watch a file

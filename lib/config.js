...
var r = {
  config : {
	...
	runVows : true, // if should run Vows
  ...
  loadConfig : function(extConfig) {
    ...
    r.config.vowsExec			= argv.vowsexec || r.config.vowsExec;
    ...
    r.config.runVows			= (argv["novows"] !== undefined) ? !def.runVows : r.config.runVows;
	...
	if (argv.vows)  {
      var vows = argv.vows;
      r.config.vows = vows	.split(",").map(function(a) { 
        return a.replace(/\s/g, ""); 
      });
	}
